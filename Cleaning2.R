...
## Christelle
## Average all variables across all years for each plant for the 11 year period. Should have 92 rows.
## Something like this will give the averages for each plant over 11 years.
tidy2.1 <- aggregate(x = tidy2[c("Electricity","SO2", "NOX", "CapStock", 
                      "Employees","HC_Coal", "HC_Oil", "HC_Gas")], 
                     by=list(PlantID=tidy2$PlantID), mean)
write.table(tidy2.1, "tidy2_a.txt")
tidy2a <- read.table("tidy2_a.txt")


## Dataset that adds all variables across all variables within a particular year across all 92 plants so that the 
## tidy dataset has 11 rows of observations of all the relevant variables.

## Something like this will give you the totals for each year. 
## For example, the total Energy used by all plants in 1990.
tidy2.2 <- aggregate(x = tidy2[c("Electricity","SO2", "NOX", "CapStock", 
                                 "Employees","HC_Coal", "HC_Oil", "HC_Gas", "Phase1")], 
                     by=list(Year=tidy2$Year), sum)
write.table(tidy2.2, file = "tidy2_b.txt")
tidy2b <- read.table("tidy2_b.txt")
